...
class Home extends CI_Controller
{
	...
	public function services()
	{
		...
		if(!empty($this->uri->segment('2'))){
            ...
            $this->data['subcategory']=$this->home->getsubcategory($category);
		}
	...
	public function show_service($id){
		...
		$dataSplit = explode(".","$id"); //Added on 28-09-2020
		$aftersplit = $dataSplit[1]; //Added on 28-09-2020
        $c_idsplit = explode("-",$dataSplit[0]);
        $cid_size = sizeof($c_idsplit);
        $this->data['category_id']=$c_idsplit[($cid_size-1)];
        $this->data['subcategory_id']=$aftersplit;
        $this->data['subcategory']=$this->home->getsubcategory($c_idsplit[($cid_size-1)]);
		$conditions['returnType'] = 'count';
		$inputs=array();
		if(isset($_POST) && !empty($_POST)){
			$inputs['price_range'] = $this->input->post('price_range'); 
        ...
        $config['link_func']      = 'getData';
        $config['loading']='<img src="'.base_url().'assets/img/loader.gif" alt="" />';
        $config['base_url']    = base_url('home/ajaxPaginationDataById2'); 
        $config['total_rows']  = $totalRec; 
        $config['per_page']    = $this->perPage; 
        // Initialize pagination library 
        $this->ajax_pagination->initialize($config); 
	...
	public function all_services()
	{
		...
		$inputs['user_address'] = $this->input->post('user_address'); 
        $totalRec = $this->home->get_all_service($conditions,$inputs);
        //echo $totalRec.json_encode($inputs); 
        // Pagination configuration 
        $config['target']      = '#dataList'; 
        $config['link_func']      = 'getData';
        $config['loading']='<img src="'.base_url().'assets/img/loader.gif" alt="" />';
    ...
    function ajaxPaginationData2(){ 
        ...
        $inputs['common_search'] = $this->input->post('common_search'); 
        $inputs['categories'] = $this->input->post('categories');
        $inputs['subcategory'] = $this->input->post('subcategoryid');
		$totalRec = $this->home->get_all_service($conditions,$inputs);
    ...
    function ajaxPaginationDataById()
    {
		// Define offset
        $page = $this->input->post('page');
        $categoryid = $this->input->post('category');
        $location = $this->input->post('location');
        $common_search = $this->input->post('common_search');
        if($page==0){
        	$start = 0;
            $end = 12;
        }else{
    ...
    function ajaxPaginationDataById2()
    {
		// Define offset
        $page = $this->input->post('page');
        $categoryid = $this->input->post('category');
        ...
        $inputs['subcategory'] = $this->input->post('subcategory');
		$totalRec = $this->home->get_all_service($conditions,$inputs);
        //echo "<script>alert('".count($totalRec)."');</script>";
        if(count($totalRec)<$end)
        {
        	$x=count($totalRec)%12;
        	$end=$page+$x;
	...
	public function current_location(){ 
  	...
  	public function clear_all_noty(){
  		if(!empty($_POST['id'])){
  			...
  			if($res==true){
  				echo json_encode(['success'=>true,'msg'=>'cleared']);exit;
  			}
  			else{
  				echo json_encode(['success'=>false,'msg'=>'not cleared']);exit;
  			}
  		}
  	}

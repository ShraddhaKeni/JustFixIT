...
class Booking extends CI_Controller {
  ...
  public function booking_service_submit()
  {
    if($this->input->post())
    {
      if($this->input->post('txStatus')=='SUCCESS')
      {
        ...
        $provider_mobileno = $getprovider['mobileno'];
        $user_mobileno = $getuser['mobileno'];
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://2factor.in/API/R1/?module=TRANS_SMS&apikey=".$api_key."&to=".$provider_mobileno."&from=AXZORA&templatename=bookedServiceProvider&var1=".$getuser['mobileno']."&var2=".$get_book_service['to_time'],       
        ...
        curl_setopt_array($curl1, array(
          CURLOPT_URL => "https://2factor.in/API/R1/?module=TRANS_SMS&apikey=".$api_key."&to=".$user_mobileno."&from=AXZORA&templatename=bookserviceUser&var1=".$getuser['mobileno']."&var2=".$usersDetail,
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
    ...
    else
    {
      ...
      $booking = [
        ...
        'user_id' => $this->session->userdata('id'),
        'token' => 'Old Type',
        'amount'=>  $records['service_amount'],
        'service_date'=>date('Y-m-d',strtotime($this->input->get('booking_date'))),
        'location'=>$this->input->get('service_location'),
        'latitude'=> $latitude,
        'longitude'=>$longitude,
        'from_time'=>$from_time,
        'to_time'=>$to_time,
  ...
  public function booking_service_submit_wallet()
  {
    $passed_data=$this->input->get('data');
    $pd = json_decode($passed_data);
    $service_id = $pd->service_id;
    $provider_id = $pd->provider_id;
    $booking_date = $pd->booking_date;
    $from_time1 = $pd->from_time;
    $service_location = $pd->service_location;
    $service_latitude = $pd->service_latitude;
    $service_longitude = $pd->service_longitude;
    $notes = $pd->notes;
    ...
    {
      $actualAmount = $walletAmt[0]->wallet_amt - $records['service_amount'];
      $waltdata = array();
      ...
      $booking_time = explode('-',$from_time1);
      $start_time = strtotime($booking_time[0]);
      $end_time = strtotime($booking_time[1]);
      $from_time = date('G:i:s',($start_time));
      $to_time = date('G:i:s',($end_time));
      ...
      $pd='{"provider_id":"'.$provider_id.'","user_id":"'.$user_id.'","status":"1","amount":"'.$records['service_amount'].'","service_title":"'.$records['service_title'].'"}';
      $history_pay['token']=$token;
      $history_pay['user_provider_id']=$user_id;
      $history_pay['type']=1;
      $history_pay['tokenid']='Old Type';
      $history_pay['payment_detail']= $pd;//response
      $history_pay['charge_id']=$service_id;
      $history_pay['transaction_id']=$bookId;
      $history_pay['exchange_rate']=0;
      $history_pay['paid_status']='pass';
      $history_pay['cust_id']='Self';
      $history_pay['card_id']='Self';
      $history_pay['total_amt']=$records['service_amount'];
      $history_pay['fee_amt']=0;
      $history_pay['net_amt']=$records['service_amount'];
      $history_pay['amount_refund']=0;
      $history_pay['current_wallet']=$walletAmt[0]->wallet_amt;
      $history_pay['credit_wallet']=0;
      $history_pay['debit_wallet']=$records['service_amount'];
